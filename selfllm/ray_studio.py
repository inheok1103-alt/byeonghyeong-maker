...
def analyze_with_brain(passage):
    log("뇌(API)에게 지문 분석 요청 중…")
    prompt = ray_escalate.lessons_preamble() + BRAIN_PROMPT.replace("{PASSAGE}", passage)
    def local():
    ...
    return ray_escalate.guarded(local, repair=repair, context=prompt[:1500], topic="지문분석")
